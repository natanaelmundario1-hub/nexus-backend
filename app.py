...
# O Python vai ler o link do banco direto das configurações seguras do Render
...
def inicializar_banco_dados():
    ...
    try:
        ...
        cursor.close()
        conn.close()
        print("🏛️ Banco de dados PostgreSQL inicializado e tabelas verificadas.")
...
def buscar_taxa_veiculos_bacen():
    ...
    try:
        # URL oficial da API do Bacen para taxa de juros de aquisição de veículos (Pessoa Física)
        url = "https://bcb.gov.br"
        
        resposta = requests.get(url, timeout=5)
        dados = resposta.json()
        
        # Captura o primeiro registro da lista (mês mais recente divulgado)
        ...
        # Taxa de juros padrão caso a API do governo fique fora do ar temporariamente
        return {
            "sucesso": False,
            "taxa_mensal": 0.0165,
            "taxa_anual": 21.6,
...
def login():
    ...
    try:
        ...
        # Consulta segura contra SQL Injection buscando o e-mail no PostgreSQL
        ...
        if registro:
            # Se o banco validou, gera o Token Único de Sessão
...
def calcular_credito():
    ...
    # Cálculo das parcelas (Modelo Francês/SAC simplificado)
    ...
    return jsonify({"mensagem": mensaje})
...
def calcular_veiculo():
    ...
    # Aciona o Robô do Banco Central para pegar as taxas em tempo real
    dados_taxa = buscar_taxa_veiculos_bacen()
    i = dados_taxa['taxa_mensal']
    
    # Cálculo de Parcelas Fixas (Fórmula da Tabela Price)
...
if __name__ == '__main__':
    # Inicializa localmente em modo de desenvolvimento caso esteja testando offline
